...
import {format} from 'date-fns'; //mine
...
class Calendar extends Component {
  ...
  render() {
    ...
    while (days.length) {
      // mine
      if (this.props.currentWeekLine) {
        const weekToRender = days.splice(0, 7);
        const weekToCompare = weekToRender.map(date=> format(date[0], 'yyyy-MM-dd'));
        if (weekToCompare.includes(this.props.selectedDate)) {
          weeks.push(this.renderWeek(weekToRender, weeks.length));
        }
      } else {
